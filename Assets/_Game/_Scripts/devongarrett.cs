...
public class devongarrett : MonoBehaviour
{
    ...
    void Update()
    {
        if (maralancaster.Length > 0 && !naomijuarez)
        {
            ...
            {
                tommyfinney.Instance.ShowApplovin();
            }
            else if (kristytrent.Length > 0)
            {
                tommyfinney.Instance.ShowInterstitialfb();
            }
            else if (caratucker.Length > 0)
            {
                tommyfinney.Instance.ShowAdUnity();
            }
            SceneManager.LoadScene(tommyfinney.Homenamescene);
    ...
    void chelseysamuel()
    {
        FirebaseDatabase.GetInstance(tommyfinney.firebaselink)
      .GetReference("MyMob")
